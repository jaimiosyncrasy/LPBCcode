    ...
    for j=1:r % for all voltages
        % ctrl_idx groups Ps, then Qs; however dbcMeas is ordered [P Q P Q...]
            ...
            elseif ~isempty(strfind(loadNames{ctrl_idx(i)},'/Q'))
                dvdqMat(i-r,j)=(vmag_new(dbcMeas(2*i-length(ctrl_idx)),j)-vmag_new(dbcMeas(2*i-length(ctrl_idx))-dbcDur,j))/(stepQ*1000/Sbase);
                dv2dqMat(i-r,j)=(vmag_new(dbcMeas(2*i-length(ctrl_idx)),j)^2-vmag_new(dbcMeas(2*i-length(ctrl_idx))-dbcDur,j)^2)/(stepQ*1000/Sbase);
                ddeldqMat(i-r,j)=(vang_new(dbcMeas(2*i-length(ctrl_idx)),j)-vang_new(dbcMeas(2*i-length(ctrl_idx))-dbcDur,j))/(stepQ*1000/Sbase);
            end
    end
end
