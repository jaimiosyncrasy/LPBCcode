function [vmag_ref,vang_ref,p_init,q_init,vmag_init_actual, vang_init_actual]  = set_const_target_fullDay(minStart,minEnd,Sbase,meas_idx,measStr,controlLoopAlign,test_num,num_tarSets) 
    ...
    [tarVmag,tarVang,Vang_nom] = process_targets_fullDay(minStart,controlLoopAlign,measStr,meas_idx,test_num,num_tarSets)
    secList=[1:(minEnd-minStart)*60]'; % starts from 1, not actual second of the 
	
    Vmag_nom=ones(1,length(unique(meas_idx,'stable')));
...
%% initialization step for that first time run RT lab initialize voltages to be steady state values
    ...
    if (length(tarVmag)~=length(unique(meas_idx,'stable')) || length(Vmag_nom)~=length(unique(meas_idx,'stable')))
