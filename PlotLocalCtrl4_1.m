...
plotEnd=minStart+2; %minEnd for entire simulation
...
%     end
    xlabel(strcat('timesteps,Ts=',num2str(Ts),'sec')); ylabel('Reactive Power (pu)');title('Q at actuator node, (Qmcd) p.u.'); grid on; legend('652_a','671_a','692_a','Qmin','Qmax');  
...
% Plot outputs v
    ...
    plot(tidx(inter+1),vmag_new(tidx(inter+1),3),'r-',tidx(inter+1),vmag_ref_sig(tidx(inter+1),3),'r-.','LineWidth',2);
    plot(tidx(inter+1),vmag_new(tidx(inter+1),4),'g-',tidx(inter+1),vmag_ref_sig(tidx(inter+1),4),'g-.','LineWidth',2);
    r=tidx(inter+1);
    Ts_bound=0.02;
...
%Plot outputs vang
    ...
    plot(tidx(inter+1),vang_new(tidx(inter+1),3),'r-',tidx(inter+1),vang_ref_sig(tidx(inter+1),3),'r-.','LineWidth',2);
    plot(tidx(inter+1),vang_new(tidx(inter+1),4),'g-',tidx(inter+1),vang_ref_sig(tidx(inter+1),4),'g-.','LineWidth',2);
    %plot(t,repmat(vmin,[1,length(t)]),'k-.',t,repmat(vmax,[1,length(t)]),'k-.','LineWidth',2);
    if (Vmag_ctrl==true && Vmag_ctrlStart>plotStart)
        plot([Vmag_ctrlStart,Vmag_ctrlStart],get(gca,'ylim'),'k-');%,[dbcStart,dbcStart],get(gca,'ylim'),'k');
    end
